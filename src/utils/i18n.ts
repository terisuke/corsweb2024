  ...
  ja: {
    ...
    works: {
      ...
      items: [
        ...
        { name: "画像・テキストを複合的に処理する生成AI SaaS基盤の開発", tag: "AI基盤開発", description: "複数のAIモデルを組み合わせ、目的に応じたコンテンツ生成を自動化するSaaS基盤。課題：画像とテキストを組み合わせたコンテンツ制作に多大な人的コストがかかっている。設計・実装：複数のマルチモーダルAIを統合し、要件に応じた出力を安定して生成するためのAPI連携基盤を設計。運用：エンドユーザーが直感的に操作できるインターフェースを提供し、継続的な機能拡張が可能な運用体制を構築。", href: null, nda: true },
        { name: "大規模アンケート収集・分析基盤の刷新と運用設計", tag: "基幹システム刷新", description: "膨大な回答データを安全かつリアルタイムに処理・分析するためのシステム刷新。課題：既存システムのパフォーマンス低下と、データ分析までのタイムラグ・運用負荷の増大。設計・実装：大量アクセスに耐えうる負荷分散と、セキュアなデータ保管ルールを設計・再実装。運用：操作ログの取得や権限管理を整え、業務で運用する際の不安を減らすデータ管理体制へ移行。", href: null, nda: true },
        { name: "業務フローに寄り添う、レガシー基幹システムのクラウド移行", tag: "基幹システム刷新", description: "老朽化したオンプレミス環境から、運用を見据えたクラウド環境への移行支援。課題：既存の業務フローが複雑化しており、単なるクラウド移行では現場の混乱が予想された。設計・実装：移行前に現場の業務課題を言語化・整理し、不要な機能を削ぎ落としたうえでクラウドアーキテクチャを設計・実装。運用：運用時に確認すべき責任範囲を明確にし、現場が迷わず使える保守・運用フローを定着。", href: null, nda: true },
        { name: "建築図面および間取りの自動生成AIの実証実験", tag: "PoC / 0→1開発", description: "建築業界の専門知識とAIを掛け合わせ、設計初期段階の図面作成を支援する検証。課題：設計初期段階における間取り案の作成に時間がかかり、顧客提案のスピードが上がらない。設計・実装：建築基準や特有の設計ルールをAIに扱わせるためのデータ整形ルールを定義し、プロトタイプを設計・実装。運用：生成された図面を人間が最終確認するフローを策定し、実務への組み込み可否を判断。", href: null, nda: true }
      ]
    },
    security: {
      title: "セキュリティ方針",
