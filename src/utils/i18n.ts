  ...
  ja: {
    ...
    works: {
      ...
      items: [
        ...
        { name: "Engineer Cafe Navigator", tag: "OSS公開実績", description: "施設情報をもとに、利用者からの質問に自然な対話で答えるAI案内システム。課題：施設に関する多様な質問への対応コストが高く、利用者が情報を探しにくい。設計・実装：公開情報やFAQを整理し、適切な回答を生成するAIチャットボット基盤を設計・実装。運用：オープンソースとして公開し、コミュニティ主導での継続的な改善・運用が可能な体制を目指した。", href: null },
        ...
        { name: "大規模アンケート収集・分析基盤の刷新と運用設計", tag: "基幹システム刷新", description: "膨大な回答データを安全かつリアルタイムに処理・分析するためのシステム刷新。課題：既存システムのパフォーマンス低下と、データ分析までのタイムラグ・運用負荷の増大。設計・実装：大量アクセスに耐えうる負荷分散と、セキュアなデータ保管ルールを設計・再実装。運用：操作ログの取得や権限管理を整え、業務で運用する際の不安を減らすデータ管理体制へ移行。", href: null, nda: true, note: "※NDAのため、企業名・製品名・一部仕様を抽象化して掲載しています。" },
        { name: "業務フローに寄り添う、レガシー基幹システムのクラウド移行", tag: "基幹システム刷新", description: "老朽化したオンプレミス環境から、運用を見据えたクラウド環境への移行支援。課題：既存の業務フローが複雑化しており、単なるクラウド移行では現場の混乱が予想された。設計・実装：移行前に現場の業務課題を言語化・整理し、不要な機能を削ぎ落としたうえでクラウドアーキテクチャを設計・実装。運用：運用時に確認すべき責任範囲を明確にし、現場が迷わず使える保守・運用フローを定着。", href: null, nda: true, note: "※機密保持の観点から、特定につながる情報を伏せて掲載しています。" },
